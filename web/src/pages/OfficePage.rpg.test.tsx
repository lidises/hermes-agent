...
describe("OfficeRpgMap", () => {
  it("renders the read-only RPG room map and mirrors every entity in a text fallback", () => {
    ...
    expect(markup).toContain("data-office-deskrpg-renderer=\"canvas\"");
    expect(markup).toContain("data-office-deskrpg-canvas-shell=\"native-2d\"");
    expect(markup).toContain("data-office-deskrpg-canvas-layer=\"tile-room-actors\"");
    expect(markup).toContain("data-office-deskrpg-canvas-fallback=\"svg-retained\"");
    expect(markup).toContain("data-office-deskrpg-canvas-mode=\"read-only\"");
    ...
    expect(markup).toContain("data-office-deskrpg-canvas-layer-contract=\"floor-room-depth-descriptors\"");
    expect(markup).toContain("data-office-deskrpg-canvas-layer-count=\"9\"");
    expect(markup).toContain("data-office-deskrpg-canvas-layer-stack=\"floor,corridor,room,tile-detail,furniture,door,sprite-shadow,sprite-body,sprite-label\"");
    expect(markup).toContain("data-office-deskrpg-canvas-depth-model=\"z-ordered-readonly\"");
    ...
    expect(markup).toContain("data-office-deskrpg-canvas-door-contract=\"door-descriptors\"");
    ...
    expect(markup).toContain("data-office-deskrpg-canvas-cue-layout-contract=\"collision-aware-mobile-offsets\"");
    expect(markup).toContain("data-office-deskrpg-canvas-cue-density-contract=\"crowded-room-cue-density\"");
    ...
    expect(markup).toContain("data-office-deskrpg-canvas-route-focus-contract=\"corridor-room-focus-descriptors\"");
    expect(markup).toContain("data-office-deskrpg-canvas-route-focus-count=\"4\"");
    expect(markup).toContain("data-office-deskrpg-canvas-room-focus-count=\"6\"");
    expect(markup).toContain("data-office-deskrpg-canvas-route-emphasis=\"read-only-corridor-glow\"");
    expect(markup).toContain("data-office-deskrpg-canvas-room-focus-mode=\"active-room-halo\"");
    expect(markup).toContain("data-office-deskrpg-canvas-viewport-contract=\"small-screen-route-cue-viewport\"");
    expect(markup).toContain("data-office-deskrpg-canvas-viewport-mode=\"compact-readable\"");
    expect(markup).toContain("data-office-deskrpg-canvas-compact-route-scale=\"0.86\"");
    expect(markup).toContain("data-office-deskrpg-canvas-cue-viewport-lane=\"mobile-edge-safe-cues\"");
    expect(markup).toContain("data-office-deskrpg-canvas-mobile-route-visibility=\"persistent-readable-routes\"");
    expect(markup).toContain("data-office-deskrpg-canvas-mobile-viewport-padding=\"24\"");
    expect(markup).toContain("data-office-deskrpg-canvas-min-cue-legibility=\"8px-bold\"");
    expect(markup).toContain("data-office-deskrpg-canvas-viewport-frame-contract=\"safe-viewport-frame-descriptors\"");
    expect(markup).toContain("data-office-deskrpg-canvas-viewport-legend-mode=\"route-cue-padding-legend\"");
    expect(markup).toContain("data-office-deskrpg-canvas-viewport-frame-padding=\"24\"");
    ...
    expect(markup).toContain("data-office-rpg-furniture-kind=\"chair\"");
    expect(markup).toContain("data-office-rpg-furniture-kind=\"meeting-table\"");
    expect(markup).toContain("data-office-rpg-furniture-kind=\"sofa\"");
    expect(markup).toContain("data-office-rpg-furniture-kind=\"plant\"");
    expect(markup).toContain("data-office-rpg-furniture-kind=\"monitor\"");
    expect(markup).toContain("data-office-rpg-furniture-kind=\"whiteboard\"");
    expect(markup).toContain("data-office-rpg-furniture-kind=\"bookcase\"");
    expect(markup).toContain("data-office-rpg-sprite-world-context=\"furniture-embedded\"");
    expect(markup).toContain("픽셀 오피스 생활감");
    expect(markup.match(/data-office-rpg-furniture-piece=/g)?.length).toBeGreaterThanOrEqual(18);
    ...
    expect(markup).toContain("data-office-rpg-sprite-route-phase=\"route-synced\"");
...
describe("NasKeeperExecutionOperatorActionPanel", () => {
  ...
  it("builds a durable queue guarded operator readiness contract with execution disabled by default", () => {
    const boundary = { detailKind: "nas_runtime_n3_approval_boundary_status_surface" } as ReturnType<typeof buildOfficeNasRuntimeN3ApprovalBoundaryStatusSurface>;
    const action = buildOfficeNasRuntimeSingleFileWriteApprovalAction(boundary);
    const queueSurface = buildOfficeNasKeeperQueueManualEvidenceReviewSurface(action);
    const operatorAction = buildOfficeNasKeeperExecutionOperatorAction(queueSurface);
    const readiness = buildOfficeNasKeeperDurableQueueGuardedOperatorReadiness(operatorAction);

    expect(readiness.stageLabel).toBe("NAS Keeper Durable Queue Guarded Operator Readiness 1");
    expect(readiness.durableQueueItemRequired).toBe(true);
    ...
    expect(readiness.macRelayPrecommitManifestReady).toBe(true);
    ...
    expect(readiness.watcherCronDaemonEnabled).toBe(false);
    expect(readiness.relayDispatchEnabled).toBe(false);
    expect(readiness.authorityAdapterBindingEnabled).toBe(false);
    expect(readiness.markdownBodyProjected).toBe(false);
  ...
  it("renders durable queue guarded operator readiness as default-disabled DOM evidence", () => {
    const Panel = (OfficePageModule as unknown as {
      NasKeeperDurableQueueGuardedOperatorReadinessPanel: React.ComponentType<{ readiness: ReturnType<typeof buildOfficeNasKeeperDurableQueueGuardedOperatorReadiness> }>;
    ...
    const markup = renderToStaticMarkup(<Panel readiness={readiness} />);

    expect(markup).toContain('data-office-nas-keeper-durable-queue-guarded-operator-readiness="true"');
    ...
    expect(markup).toContain('data-office-nas-keeper-durable-queue-guarded-operator-mac-relay-precommit-manifest-ready="true"');
...
describe("Office controlled mutation compact dashboard", () => {
  ...
  it("summarizes latest Mac relay precommit manifest above precommit metadata without unsafe echo", () => {
    const CompactPanel = (OfficePageModule as unknown as {
      OfficeControlledMutationCompactDashboardPanel: React.ComponentType<{
        latestPrecommitMetadata?: { found?: boolean; dto?: Record<string, unknown> | null; latest_record?: Record<string, unknown> | null } | null;
    ...
    const forbiddenBody = ["manifest", "body", "must", "not", "echo"].join("-");
    const forbiddenPath = ["", "Users", "private", "nas-manifest"].join("/");
    const forbiddenSecret = ["sk", "manifest-secret"].join("-");
    const markup = renderToStaticMarkup(
      <CompactPanel
        latestPrecommitMetadata={{
          found: true,
    ...
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-precommit-manifest-ready="true"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-precommit-manifest-source-verified="true"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-precommit-manifest-replay-store-write="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-precommit-manifest-real-write="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-precommit-manifest-vps-authority="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-precommit-manifest-runtime-open="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-precommit-manifest-payload-echo="false"');
    expect(markup).toContain('Mac relay precommit manifest');
    expect(markup).toContain('precommitmanifest-safe-ref');
    expect(markup).not.toContain('precommit-safe-ref</div>');
    expect(markup).not.toContain('historical ladder detail');
    expect(markup).not.toContain(forbiddenBody);
    expect(markup).not.toContain(forbiddenPath);
    expect(markup).not.toContain(forbiddenSecret);
  ...
  it("summarizes latest Mac relay production-write approval above approval token without unsafe echo", () => {
    const CompactPanel = (OfficePageModule as unknown as {
      OfficeControlledMutationCompactDashboardPanel: React.ComponentType<{
        latestApprovalToken?: { found?: boolean; dto?: Record<string, unknown> | null; latest_record?: Record<string, unknown> | null } | null;
    ...
    const forbiddenBody = ["production", "approval", "must", "not", "echo"].join("-");
    const forbiddenPath = ["", "Users", "private", "prod-approval"].join("/");
    const forbiddenSecret = ["sk", "prod-approval-secret"].join("-");
    const markup = renderToStaticMarkup(
      <CompactPanel
        latestApprovalToken={{
          found: true,
    ...
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-production-write-approval-ready="true"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-production-write-approval-source-verified="true"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-production-write-approval-replay-store-write="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-production-write-approval-real-write="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-production-write-approval-vps-authority="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-production-write-approval-runtime-open="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-production-write-approval-payload-echo="false"');
    expect(markup).toContain('Mac relay production-write approval');
    expect(markup).toContain('prodapproval-safe-ref');
    expect(markup).not.toContain('approvaltoken-safe-ref</div>');
    expect(markup).not.toContain('historical ladder detail');
    expect(markup).not.toContain(forbiddenBody);
    expect(markup).not.toContain(forbiddenPath);
    expect(markup).not.toContain(forbiddenSecret);
  ...
  it("summarizes latest Mac relay real NAS write execution envelope above dry-run seal without unsafe echo", () => {
    const CompactPanel = (OfficePageModule as unknown as {
      OfficeControlledMutationCompactDashboardPanel: React.ComponentType<{
        latestDryRunSeal?: { found?: boolean; dto?: Record<string, unknown> | null; latest_record?: Record<string, unknown> | null } | null;
        latestExecutionEnvelope?: { found?: boolean; dto?: Record<string, unknown> | null; latest_record?: Record<string, unknown> | null } | null;
        detailCount: number;
        children: React.ReactNode;
      }>;
    }).OfficeControlledMutationCompactDashboardPanel;
    const forbiddenBody = ["execution", "envelope", "must", "not", "echo"].join("-");
    const forbiddenPath = ["", "Users", "private", "nas-execution-envelope"].join("/");
    const forbiddenSecret = ["sk", "execution-envelope-secret"].join("-");
    const markup = renderToStaticMarkup(
      <CompactPanel
        latestDryRunSeal={{
          found: true,
        ...
        latestExecutionEnvelope={{
          found: true,
          dto: {
            write_readiness_percent: 100,
            mac_relay_real_nas_write_execution_envelope_ref: "nasexecenv-safe-ref",
            mac_relay_real_nas_write_execution_envelope_ready: true,
            source_mac_relay_real_nas_write_dry_run_seal_verified: true,
            source_dry_run_seal_contract_verified: true,
            target_filename_contract_verified: true,
    ...
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-execution-envelope-ready="true"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-execution-envelope-source-verified="true"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-execution-envelope-replay-store-write="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-execution-envelope-real-write="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-execution-envelope-vps-authority="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-execution-envelope-runtime-open="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-execution-envelope-payload-echo="false"');
    expect(markup).toContain('Mac relay real NAS write execution envelope');
    expect(markup).toContain('nasexecenv-safe-ref');
    expect(markup).not.toContain('nasdryrunseal-safe-ref</div>');
    expect(markup).not.toContain('historical ladder detail');
    expect(markup).not.toContain(forbiddenBody);
    expect(markup).not.toContain(forbiddenPath);
    expect(markup).not.toContain(forbiddenSecret);
  ...
  it("summarizes latest Mac relay real NAS write final execution gate above execution envelope without unsafe echo", () => {
    const CompactPanel = (OfficePageModule as unknown as {
      OfficeControlledMutationCompactDashboardPanel: React.ComponentType<{
        latestExecutionEnvelope?: { found?: boolean; dto?: Record<string, unknown> | null; latest_record?: Record<string, unknown> | null } | null;
    ...
    const forbiddenBody = ["final", "execution", "gate", "must", "not", "echo"].join("-");
    const forbiddenPath = ["", "Users", "private", "nas-final-execution-gate"].join("/");
    const forbiddenSecret = ["sk", "final-execution-gate-secret"].join("-");
    const markup = renderToStaticMarkup(
      <CompactPanel
        latestExecutionEnvelope={{
          found: true,
    ...
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-final-execution-gate-ready="true"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-final-execution-gate-source-verified="true"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-final-execution-gate-replay-store-write="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-final-execution-gate-real-write="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-final-execution-gate-vps-authority="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-final-execution-gate-runtime-open="false"');
    expect(markup).toContain('data-office-controlled-mutation-compact-dashboard-mac-relay-real-nas-write-final-execution-gate-payload-echo="false"');
    expect(markup).toContain('Mac relay real NAS write final execution gate');
    expect(markup).toContain('nasfinalgate-safe-ref');
    expect(markup).not.toContain('nasexecenv-safe-ref</div>');
    expect(markup).not.toContain('historical ladder detail');
    expect(markup).not.toContain(forbiddenBody);
    expect(markup).not.toContain(forbiddenPath);
    expect(markup).not.toContain(forbiddenSecret);
