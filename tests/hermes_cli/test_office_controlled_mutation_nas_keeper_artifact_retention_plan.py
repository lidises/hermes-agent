...
def cleanup_execution_summary_receipt_payload(disabled_checksum: str, **overrides):
    payload = {
        "cleanup_summary_ref": "cleanupsummary-20260527-artifact-retention-1",
        "cleanup_disabled_run_ref": "cleanupdisabled-20260527-artifact-retention-1",
        "cleanup_disabled_run_sha256": disabled_checksum,
        "exported_by": "agent_nas_keeper",
        "exported_at": "2026-05-27T11:35:00Z",
        "operator_confirmation": "metadata-only-execution-summary-no-delete-no-move-no-write",
        "summary_scope": "operator_facing_refs_checksums_capabilities_only",
        "included_refs": [
            "cleanupplan-20260527-fresh-write-artifacts",
            "cleanupgate-20260527-retention-plan-1",
            "cleanuphold-20260527-artifact-retention-1",
            "cleanupmanifest-20260527-artifact-retention-1",
            "cleanupfinal-20260527-artifact-retention-1",
            "cleanuppackage-20260527-artifact-retention-1",
            "cleanupdisabled-20260527-artifact-retention-1",
        ],
        "evidence_refs": ["sha:fdd01554e34264a023efbf4c3ad1b76bf4dd73cb97db1a8190e43640a1f1111e"],
    }
    payload.update(overrides)
...
def seed_cleanup_disabled_run_receipt(tmp_path):
    from hermes_cli.office_controlled_mutation import append_office_controlled_mutation_nas_keeper_cleanup_disabled_run_receipt
    ...
    return disabled_path, disabled


def test_cleanup_execution_summary_receipt_exports_refs_only_and_is_idempotent(tmp_path):
    from hermes_cli.office_controlled_mutation import append_office_controlled_mutation_nas_keeper_cleanup_execution_summary_receipt

    disabled_path, disabled = seed_cleanup_disabled_run_receipt(tmp_path)
    summary_path = tmp_path / "cleanup-execution-summary-receipts.jsonl"
    disabled_checksum = disabled["dto"]["cleanup_disabled_run_sha256"]

    first = append_office_controlled_mutation_nas_keeper_cleanup_execution_summary_receipt(
        cleanup_execution_summary_receipt_payload(disabled_checksum),
        disabled_store_path=disabled_path,
        summary_store_path=summary_path,
    )
    second = append_office_controlled_mutation_nas_keeper_cleanup_execution_summary_receipt(
        cleanup_execution_summary_receipt_payload(disabled_checksum),
        disabled_store_path=disabled_path,
        summary_store_path=summary_path,
    )
    ...
    assert first["errors"] == []
    dto = first["dto"]
    assert dto["mode"] == "nas_keeper_cleanup_execution_summary_receipt_recorded"
    assert dto["metadata_only_record_write"] is True
    assert dto["disabled_run_checksum_matched"] is True
    assert dto["operator_facing_export"] is True
    assert dto["refs_only_export"] is True
    assert dto["included_ref_count"] == 7
    assert len(dto["cleanup_summary_sha256"]) == 64
    assert dto["cleanup_execution_opened"] is False
...
def test_cleanup_execution_summary_receipt_requires_exact_disabled_checksum_and_rejects_raw_fields_without_echo(tmp_path):
    from hermes_cli.office_controlled_mutation import append_office_controlled_mutation_nas_keeper_cleanup_execution_summary_receipt

    disabled_path, disabled = seed_cleanup_disabled_run_receipt(tmp_path)
    summary_path = tmp_path / "cleanup-execution-summary-receipts.jsonl"

    mismatch = append_office_controlled_mutation_nas_keeper_cleanup_execution_summary_receipt(
        cleanup_execution_summary_receipt_payload("0" * 64),
        disabled_store_path=disabled_path,
        summary_store_path=summary_path,
    )
    assert mismatch["stored"] is False
    assert mismatch["dto"] is None
    assert mismatch["errors"] == [{"field": "cleanup_disabled_run_sha256", "code": "cleanup_disabled_run_checksum_mismatch"}]

    raw = append_office_controlled_mutation_nas_keeper_cleanup_execution_summary_receipt(
        cleanup_execution_summary_receipt_payload(disabled["dto"]["cleanup_disabled_run_sha256"], raw_root_path="/" + "Users/lidises/private"),
        disabled_store_path=disabled_path,
        summary_store_path=summary_path,
    )
    assert raw["stored"] is False
...
def test_cleanup_execution_summary_receipt_api_requires_session_and_stays_metadata_only(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    from hermes_cli.office_controlled_mutation import (
        append_office_controlled_mutation_nas_keeper_artifact_retention_plan,
        append_office_controlled_mutation_nas_keeper_cleanup_execution_gate,
    ...
    client = TestClient(app)
    route = "/api/office/controlled-mutation/nas-runtime/nas-keeper-cleanup-execution-summary-receipt"
    payload = cleanup_execution_summary_receipt_payload(disabled["dto"]["cleanup_disabled_run_sha256"])
    ...
    assert body["stored"] is True
    assert body["dto"]["metadata_only_record_write"] is True
    assert body["dto"]["refs_only_export"] is True
...
def cleanup_closure_receipt_payload(summary_checksum: str, **overrides):
    payload = {
        "cleanup_closure_ref": "cleanupclosure-20260527-artifact-retention-1",
        "cleanup_summary_ref": "cleanupsummary-20260527-artifact-retention-1",
        "cleanup_summary_sha256": summary_checksum,
        "closed_by": "agent_nas_keeper",
        "closed_at": "2026-05-27T11:55:00Z",
        "operator_confirmation": "metadata-only-closure-no-execution-authority-no-delete-no-move-no-write",
        "closure_reason": "no-authority-checkpoint-before-real-cleanup",
        "required_next_approval": "separate_exact_cleanup_execution_approval",
        "evidence_refs": ["sha:fdd01554e34264a023efbf4c3ad1b76bf4dd73cb97db1a8190e43640a1f1111e"],
    }
    payload.update(overrides)
    return payload
...
def seed_cleanup_summary_receipt(tmp_path):
    from hermes_cli.office_controlled_mutation import append_office_controlled_mutation_nas_keeper_cleanup_execution_summary_receipt
...
def test_cleanup_closure_receipt_seals_no_authority_checkpoint_and_is_idempotent(tmp_path):
    ...
    summary_checksum = summary["dto"]["cleanup_summary_sha256"]

    first = append_office_controlled_mutation_nas_keeper_cleanup_closure_receipt(
        cleanup_closure_receipt_payload(summary_checksum),
        summary_store_path=summary_path,
        closure_store_path=closure_path,
    )
    second = append_office_controlled_mutation_nas_keeper_cleanup_closure_receipt(
        cleanup_closure_receipt_payload(summary_checksum),
        summary_store_path=summary_path,
        closure_store_path=closure_path,
    )
    ...
    assert first["errors"] == []
    dto = first["dto"]
    assert dto["mode"] == "nas_keeper_cleanup_closure_receipt_recorded"
    assert dto["metadata_only_record_write"] is True
    assert dto["summary_checksum_matched"] is True
    assert dto["closure_terminal"] is True
    assert dto["no_authority_checkpoint"] is True
    assert dto["write_readiness"] == "100_percent_pre_real_cleanup_boundary"
    assert len(dto["cleanup_closure_sha256"]) == 64
    assert dto["execution_authority_created"] is False
    assert dto["cleanup_execution_opened"] is False
    assert dto["actual_nas_delete_enabled"] is False
    assert dto["actual_nas_move_enabled"] is False
...
def test_cleanup_closure_receipt_requires_exact_summary_checksum_and_rejects_raw_fields_without_echo(tmp_path):
    from hermes_cli.office_controlled_mutation import append_office_controlled_mutation_nas_keeper_cleanup_closure_receipt

    summary_path, summary = seed_cleanup_summary_receipt(tmp_path)
    closure_path = tmp_path / "cleanup-closure-receipts.jsonl"

    mismatch = append_office_controlled_mutation_nas_keeper_cleanup_closure_receipt(
        cleanup_closure_receipt_payload("0" * 64),
        summary_store_path=summary_path,
        closure_store_path=closure_path,
    )
    assert mismatch["stored"] is False
    assert mismatch["dto"] is None
    assert mismatch["errors"] == [{"field": "cleanup_summary_sha256", "code": "cleanup_summary_checksum_mismatch"}]

    raw = append_office_controlled_mutation_nas_keeper_cleanup_closure_receipt(
        cleanup_closure_receipt_payload(summary["dto"]["cleanup_summary_sha256"], raw_root_path="/" + "Users/lidises/private"),
        summary_store_path=summary_path,
        closure_store_path=closure_path,
    )
    assert raw["stored"] is False
...
def test_cleanup_closure_receipt_api_requires_session_and_stays_no_authority(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    from hermes_cli.office_controlled_mutation import (
        append_office_controlled_mutation_nas_keeper_artifact_retention_plan,
        append_office_controlled_mutation_nas_keeper_cleanup_execution_gate,
    ...
    client = TestClient(app)
    route = "/api/office/controlled-mutation/nas-runtime/nas-keeper-cleanup-closure-receipt"
    payload = cleanup_closure_receipt_payload(summary["dto"]["cleanup_summary_sha256"])
    ...
    assert body["stored"] is True
    assert body["dto"]["metadata_only_record_write"] is True
    assert body["dto"]["closure_terminal"] is True
    assert body["dto"]["no_authority_checkpoint"] is True
    assert body["dto"]["execution_authority_created"] is False
    assert body["dto"]["cleanup_execution_opened"] is False
    assert body["dto"]["actual_nas_delete_enabled"] is False
    assert body["dto"]["actual_nas_move_enabled"] is False
...
def seed_step10_completion_receipt(tmp_path):
    from hermes_cli.office_controlled_mutation import append_office_controlled_mutation_nas_keeper_tmp_root_step10_completion_receipt
...
def test_step11_read_only_status_aggregate_returns_counts_and_safe_capabilities_without_raw_records(tmp_path):
    from hermes_cli.office_controlled_mutation import build_office_controlled_mutation_nas_keeper_step11_read_only_status_aggregate

    completion_path, completion = seed_step10_completion_receipt(tmp_path)
    result = build_office_controlled_mutation_nas_keeper_step11_read_only_status_aggregate(
        store_paths={"step10_completion": completion_path}
    )

    assert result["found"] is True
    assert result["errors"] == []
    dto = result["dto"]
    assert dto["mode"] == "nas_keeper_step11_read_only_status_aggregate"
    assert dto["status_surface"] == "read_only_rendering"
    assert dto["readiness_percent"] == 100
    assert dto["ready_for_read_only_rendering"] is True
    assert dto["latest_step10_completion_ref"] == completion["dto"]["tmp_root_completion_ref"]
    assert dto["latest_step10_completion_sha256"] == completion["dto"]["tmp_root_completion_sha256"]
    assert dto["record_counts"]["step10_completion"] == 1
    assert dto["record_counts"]["retention_plan"] == 0
    assert dto["ladder_steps"][-2] == {"id": "step10_completion", "label": "Step 10 completion", "record_count": 1, "complete": True}
    assert dto["ladder_steps"][-1] == {"id": "step11_hydration_receipt", "label": "Step 11 hydration receipt", "record_count": 0, "complete": False}
    assert dto["cleanup_execution_opened"] is False
    assert dto["execution_authority_created"] is False
    assert dto["real_nas_production_write_enabled"] is False
    assert dto["direct_vps_nas_write_enabled"] is False
...
def test_step11_read_only_status_aggregate_api_requires_session_and_hides_records(tmp_path, monkeypatch):
    ...
    client = TestClient(app)
    route = "/api/office/controlled-mutation/nas-runtime/nas-keeper-step11-read-only-status"
    ...
    assert dto["record_counts"] == {
        ...
        "closure_receipt": 1,
        "step10_completion": 1,
        "step11_hydration_receipt": 0,
    }
    assert dto["readiness_percent"] == 100
    assert dto["next_required_boundary"] == "read_only_status_rendering_hydrated"
    assert dto["execution_authority_created"] is False
    assert dto["real_nas_production_write_enabled"] is False
    assert dto["direct_vps_nas_write_enabled"] is False
    assert "records" not in dto
...
def step11_hydration_receipt_payload(step10_ref="tmpcompletion-20260527-step10-before-rendering-1", step10_sha=None, aggregate_sha=None):
    return {
        "step11_hydration_receipt_ref": "step11hydration-20260527-hydrated-status-1",
        "latest_step10_completion_ref": step10_ref,
        "latest_step10_completion_sha256": step10_sha or "a" * 64,
        "aggregate_sha256": aggregate_sha or "b" * 64,
        "hydrated_status_surface": "read_only_rendering",
        "readiness_percent": 100,
        "hydrated_by": "operator-step11-hydrator",
        "hydrated_at": "2026-05-27T12:59:00Z",
        "operator_confirmation": "step11-read-only-status-aggregate-hydrated",
        "next_stage": "step11_hydration_receipt_recorded",
        "evidence_refs": ["sha:fdd01554e34264a023efbf4c3ad1b76bf4dd73cb97db1a8190e43640a1f1111e"],
    }


def test_step11_hydration_receipt_append_is_metadata_only_and_idempotent(tmp_path):
    from hermes_cli.office_controlled_mutation import (
    ...
    first = append_office_controlled_mutation_nas_keeper_step11_hydration_receipt(
        payload,
        completion_store_path=completion_path,
        hydration_receipt_store_path=receipt_path,
    )
    second = append_office_controlled_mutation_nas_keeper_step11_hydration_receipt(
        payload,
        completion_store_path=completion_path,
        hydration_receipt_store_path=receipt_path,
    )
    readback = list_office_controlled_mutation_nas_keeper_step11_hydration_receipt_records(hydration_receipt_store_path=receipt_path)

    assert first["stored"] is True
    assert first["idempotent_replay"] is False
    dto = first["dto"]
    assert dto["mode"] == "nas_keeper_step11_hydration_receipt_recorded"
    assert dto["metadata_only_record_write"] is True
    assert dto["readiness_percent"] == 100
    assert dto["read_only_rendering_hydrated"] is True
    assert dto["real_nas_production_write_enabled"] is False
    assert dto["direct_vps_nas_write_enabled"] is False
    assert dto["watcher_enabled"] is False
    assert dto["cron_enabled"] is False
...
def test_step11_hydration_receipt_rejects_mismatched_or_raw_payload_without_echo(tmp_path):
    from hermes_cli.office_controlled_mutation import append_office_controlled_mutation_nas_keeper_step11_hydration_receipt
    ...
    payload["raw_path"] = private_path
    payload["latest_step10_completion_sha256"] = "d" * 64

    result = append_office_controlled_mutation_nas_keeper_step11_hydration_receipt(
        payload,
        completion_store_path=completion_path,
        hydration_receipt_store_path=tmp_path / "receipts.jsonl",
    )

    assert result["stored"] is False
    assert result["dto"] is None
    codes = {(item["field"], item["code"]) for item in result["errors"]}
    assert ("unsupported_fields", "unsupported_field") in codes
    assert ("latest_step10_completion_sha256", "step10_completion_checksum_mismatch") in codes
    serialized = json.dumps(result, sort_keys=True)
    assert private_path not in serialized


def test_step11_hydration_receipt_api_requires_session_and_folds_into_aggregate(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    from hermes_cli.office_controlled_mutation import (
        append_office_controlled_mutation_nas_keeper_tmp_root_step10_completion_receipt,
        build_office_controlled_mutation_nas_keeper_step11_read_only_status_aggregate,
    ...
    payload = step11_hydration_receipt_payload(
        aggregate["dto"]["latest_step10_completion_ref"],
        aggregate["dto"]["latest_step10_completion_sha256"],
        aggregate["dto"]["aggregate_sha256"],
    )

    client = TestClient(app)
    route = "/api/office/controlled-mutation/nas-runtime/nas-keeper-step11-hydration-receipt"
    ...
    assert readback.status_code == 200
    assert readback.json()["dto"]["record_count"] == 1
    status = client.get("/api/office/controlled-mutation/nas-runtime/nas-keeper-step11-read-only-status", headers={_SESSION_HEADER_NAME: _SESSION_TOKEN})
