"""Tests for NAS evidence package contract-only boundary."""
...
def test_nas_evidence_package_contract_is_contract_only_and_non_writing():
    from hermes_cli.office_controlled_mutation import build_office_controlled_mutation_nas_evidence_package_contract

    contract = build_office_controlled_mutation_nas_evidence_package_contract()
...
def test_nas_evidence_package_contract_ignores_raw_material():
    from hermes_cli.office_controlled_mutation import build_office_controlled_mutation_nas_evidence_package_contract
    ...
    contract = build_office_controlled_mutation_nas_evidence_package_contract(
        unsafe_examples={
            ...
            "review_comment": "raw reviewer comment must not echo",
...
def test_nas_evidence_package_schema_api_requires_dashboard_session_token():
    from hermes_cli.web_server import app

    unauth_client = TestClient(app)
    resp = unauth_client.get("/api/office/controlled-mutation/nas-evidence-package/schema")

    assert resp.status_code == 401


def test_nas_evidence_package_schema_api_is_protected_contract_only_route():
    from hermes_cli.web_server import app, _PUBLIC_API_PATHS, _SESSION_HEADER_NAME, _SESSION_TOKEN

    route = "/api/office/controlled-mutation/nas-evidence-package/schema"
    ...
    assert payload["mode"] == "nas_evidence_package_contract_only"
    assert payload["package"]["package_persistence_enabled"] is False
    assert payload["package"]["rollback_point_creation_enabled"] is False
    assert payload["package"]["nas_path_resolution_enabled"] is False
    assert payload["package"]["nas_mount_access_enabled"] is False
    assert payload["capabilities"]["storage_write_enabled"] is False
    ...
    assert payload["package_endpoints"] == []
    assert payload["storage_endpoints"] == []
    assert payload["nas_endpoints"] == []


def test_nas_evidence_package_schema_api_rejects_mutation_methods():
