## Current status — Stage 13 DeskRPG Canvas Phase B0 verified locally (2026-05-28T10:00Z)
...
- The Canvas shell now declares contract hooks consumed by the renderer: `data-office-deskrpg-canvas-projection="tile-sprite-contract"`, projection source `sanitized-scene`, tile grid `room-local-coordinates`, room count 6, tile count 87, sprite contract `placeholder-descriptors`, sprite count 6, and contract version `phase-b0-readonly`.
- The renderer still draws placeholder shapes only and continues to retain the SVG fallback after the Canvas surface.
...
- Commit/push and deploy Phase B0 to dashboard/core only: sync both VPS worktrees, rsync Mac-built ignored `hermes_cli/web_dist/`, verify relative hash, restart only dashboard/core services, then protected API/DOM/visual smoke. Gateway remains untouched.
