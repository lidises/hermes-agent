...
- VPS dashboard worktree was fast-forwarded to `376d2b0c` for code/docs availability only; no service restart was performed. VPS generator smoke with the shared Hermes venv passed, and both `hermes-agent-dashboard.service` and `hermes-gateway.service` remained active.
