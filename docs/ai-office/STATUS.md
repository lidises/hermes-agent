## Current status — Stage 13 DeskRPG Canvas Phase B2 verified locally (2026-05-28T10:38Z)
...
- The Canvas shell now declares sprite detail hooks: `data-office-deskrpg-canvas-silhouette-contract="head-body-feet-shadow"`, silhouette count 6, nameplate contract/count 6, status cue contract/count 6, sprite detail level `silhouette-nameplate`, and contract version `phase-b2-readonly`.
- The renderer now draws head/body/feet/shadow/nameplate/status-dot sprite cues from sanitized scene entities while retaining typed tile/furniture/door/corridor descriptors and the SVG fallback.
...
- Commit/push and deploy Phase B2 to dashboard/core only: sync both VPS worktrees, rsync Mac-built ignored `hermes_cli/web_dist/`, verify relative hash, restart only dashboard/core services, then protected API/DOM/visual smoke. Gateway remains untouched.
