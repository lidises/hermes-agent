...
After `Worker Facility Readiness 1`, `Worker Assignment Candidate Gate 1` is now locally implemented: a blocked/display-only candidate posture for routed worker facilities (`orchestrator_desk`, `agent_desks`, `incident_corner`) that shows why assignment candidacy cannot advance. The next separately approved work should be `Worker Request Draft Preview 1`: a hypothetical safe request-draft preview derived from blocked candidates, without assigning work, creating or persisting requests, enqueueing real work, dispatching actions, writing audit events, enabling controls, adding backend mutation routes, or changing services.
