...
After `Worker Assignment Candidate Gate 1`, `Worker Request Draft Preview 1` is now locally implemented: a hypothetical/display-only request draft posture for routed worker facilities (`orchestrator_desk`, `agent_desks`, `incident_corner`) that shows safe draft fields without creating or persisting requests. The next separately approved work should be `Worker Human Confirmation Envelope 1`: a read-only confirmation envelope showing what human confirmation metadata would be required before a draft can become actionable, without recording decisions, assigning work, creating or persisting requests, enqueueing real work, dispatching actions, writing audit events, enabling controls, adding backend mutation routes, or changing services.
