...
After `Worker Dispatch Dry-Run Envelope 1`, `Worker Audit Preview Envelope 1` is now locally implemented: a read-only audit preview envelope for routed worker dry-run envelopes (`orchestrator_desk`, `agent_desks`, `incident_corner`) that shows required dry-run reference, audit sink, event type, redaction policy, and rollback reference without writing audit events. The next separately approved work should be `Worker Rollback Preview Envelope 1`: a read-only rollback preview envelope showing what rollback metadata would be required after audit preview before reversible execution can exist, without executing rollback, writing audit events, executing actions, dispatching actions, assigning work, creating or persisting requests, enqueueing real work, enabling controls, adding backend mutation routes, or changing services.
