...
After `Desk RPG Room Shell 1`, `Desk RPG Inspector Migration 1` is now locally implemented: `/office` bridges the safe Desk RPG DTO into a right-inspector posture with stable hooks for aggregate-only targets and board/evidence/vault/ops posture, `enabledControls: 0`, `safeProjectionOnly: true`, and `rawExcluded: true`. The next separately approved work should be `Desk RPG Board Evidence Tab 1`: migrate safe board/evidence details into a central board tab/posture, without enabling execution, granting authority, recording approvals, executing rollback, writing audit events, running dry-runs, creating or persisting proposals/requests, enabling controls, dispatching actions, assigning work, enqueueing real work, adding backend mutation routes, adding renderer dependencies, or changing services.
