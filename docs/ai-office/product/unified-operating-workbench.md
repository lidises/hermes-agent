...
After `Implementation Roadmap 1`, `Desk RPG Projection ViewModel Helper 1` is now locally implemented: `buildOfficeDeskRpgProjectionModel(state)` produces safe actors, facilities, board/evidence/vault/ops posture, inspector targets, suppression counts, and redaction metadata with `enabledControls: 0`, `safeProjectionOnly: true`, and `rawExcluded: true`. The next separately approved work should be `Desk RPG Room Shell 1`: render this DTO as a read-only DOM/CSS operating-room shell, without enabling execution, granting authority, recording approvals, executing rollback, writing audit events, running dry-runs, creating or persisting proposals/requests, enabling controls, dispatching actions, assigning work, enqueueing real work, adding backend mutation routes, adding renderer dependencies, or changing services.
