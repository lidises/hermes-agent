...
After `Worker Authority Handoff Envelope 1`, `Worker Dispatch Dry-Run Envelope 1` is now locally implemented: a read-only dry-run envelope for routed worker handoffs (`orchestrator_desk`, `agent_desks`, `incident_corner`) that shows required simulation scope, expected effects, rollback plan, and audit preview references without executing or dispatching actions. The next separately approved work should be `Worker Audit Preview Envelope 1`: a read-only audit preview envelope showing what audit metadata would be required after dry-run review before audit write can exist, without writing audit events, executing actions, dispatching actions, assigning work, creating or persisting requests, enqueueing real work, enabling controls, adding backend mutation routes, or changing services.
