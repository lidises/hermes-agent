...
- Current requested rung: next Stage 13 visual slice should improve mobile/small-screen room-block patrol readability before adding broader external summaries: keep actors visibly bounded inside room blocks, reduce crowding of room-local patrol/name/status cues, and preserve read-only map-first posture. Do not force-replay failed_guarded or already-succeeded queue items.
