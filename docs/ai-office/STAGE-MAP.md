...
- Latest local rung: Stage 13 actor/facility grouping cues were added inside the primary SVG RPG map: each room group now declares visible-actor grouping, per-room facility zone, and compact Korean grouping label without expanding external panels.
- Latest completed/deployed rung: Stage 13 mobile label readability hooks were added inside the primary SVG RPG map and deployed/smoked on the VPS dashboard from `84eab5e21`: each room group now declares `mobile-readable` label stacking, per-room safe zone, max Korean label length, and protected mobile label baseline; label hooks hydrated, primary RPG map visible, controls inside visual map 0, raw leak false, console errors 0.
