...
`Unified Workbench IA/Layout 1`, `Approval Request View 1`, `Approval Audit Timeline 1`, `Approval Execution Gate 1`, `Authority Adapter Contract 1`, `Orchestrator Mediation Queue 1`, `Worker Intent Routing 1`, `Worker Facility Readiness 1`, `Worker Assignment Candidate Gate 1`, `Worker Request Draft Preview 1`, `Worker Human Confirmation Envelope 1`, `Worker Authority Handoff Envelope 1`, `Worker Dispatch Dry-Run Envelope 1`, `Worker Audit Preview Envelope 1`, `Worker Rollback Preview Envelope 1`, `Worker Final Gate Checklist 1`, `Controlled Mutation Proposal Contract 1`, `Controlled Mutation Dry-Run Plan 1`, `Controlled Mutation Audit Sink Plan 1`, `Controlled Mutation Rollback Verification Plan 1`, `Controlled Mutation Human Approval Plan 1`, `Controlled Mutation Authority Summary 1`, `Controlled Mutation Execution Readiness Summary 1`, `Desk RPG Projection ViewModel Helper 1`, `Desk RPG Room Shell 1`, and `Desk RPG Inspector Migration 1` are locally implemented as pure safe view-model helpers or display-only sections. The next architecture/product step should be `Desk RPG Board Evidence Tab 1`:

1. Use `buildOfficeDeskRpgProjectionModel(...)` plus existing safe board/evidence helpers to expose central-board evidence posture as a tab/section inside the Desk RPG operating room.
2. Add focused coverage for stable `data-office-desk-rpg-board-*` hooks, aggregate-only metadata, zero forms, no mutation/action controls, `enabledControls: 0`, `safeProjectionOnly: true`, and private sentinel exclusion.
3. Keep this UI display-only: no backend route/schema, renderer dependency, browser mutation controls, persistence, assignment, dispatch, approval recording, audit write, dry-run execution, rollback execution, NAS write, VPS/service/cron/gateway/public exposure, or raw projection.
4. Run focused/full frontend checks, build, diff-check, and local-only browser smoke before commit.
