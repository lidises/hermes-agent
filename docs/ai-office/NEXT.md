...
`Worker Facility Readiness 1` is now implemented locally after `Worker Intent Routing 1`. It adds `buildOfficeWorkerFacilityReadiness(routing)`, a read-only readiness posture for the routed target facilities (`orchestrator_desk`, `agent_desks`, `incident_corner`) with missing prerequisites only. The slice keeps `enabledControls: 0`, `workAssignmentEnabled: false`, `requestCreationEnabled: false`, `dispatchEnabled: false`, and `auditWriteEnabled: false`; it creates no requests, assigns no work, enqueues nothing, dispatches nothing, writes no audit events, adds no mutation routes, services, forms, buttons, backend writes, Kanban transitions, projection promote/reject operations, watcher/cron enablement, service restarts, public exposure, renderer dependencies, or raw projection. The next product-improving slice should be `Worker Assignment Candidate Gate 1`: show whether a routed intent could become an assignment candidate, still blocked/read-only until readiness, approval, audit, and authority gates exist.
