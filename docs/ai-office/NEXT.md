...
Last updated: 2026-05-15 18:12 KST
...
Current local slice: `Character Facility Boundary Strip 1` is implemented frontend-only/read-only. It adds `buildOfficeCharacterFacilityBoundaryStrip(legend, overlay)` and `CharacterFacilityBoundaryStripPanel`, composing only the safe `Character Facility Role Legend 1` and `Character State Room Overlay 1` DTOs into a six-zone mutation-boundary strip. Stable hooks include `data-office-character-facility-boundary-strip*`, per-zone hooks, and per-boundary hooks for `instruction intake disabled`, `mediation write disabled`, `assignment disabled`, `inspector write disabled`, `draft creation disabled`, and `NAS save disabled`. It keeps enabledControls=0, clickHandlerEnabled=false, keyboardHandlerEnabled=false, formControlEnabled=false, eventPersistenceEnabled=false, backendStreamEnabled=false, animationStatePersistenceEnabled=false, requestCreationEnabled=false, workAssignmentEnabled=false, dispatchEnabled=false, auditWriteEnabled=false, nasSaveEnabled=false, safeProjectionOnly=true, and rawExcluded=true. No backend/schema/API route/service/Kanban/cron/VPS/NAS change was made. Next recommended frontend-only/read-only slice after verification/commit: `Character Facility Source Ledger Strip 1` — add a compact safe strip tying the six facility zones back to aggregate source/safety provenance from existing safe DTOs only, without exposing raw prompt/task body/transcript/path/source title/provider and without adding click handlers, keyboard handlers, forms/buttons/inputs, event persistence, backend streams, animation-state persistence, request creation, assignment, dispatch, audit writes, or NAS saves.
