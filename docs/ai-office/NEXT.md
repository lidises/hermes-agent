...
`Worker Assignment Candidate Gate 1` is now implemented locally after `Worker Facility Readiness 1`. It adds `buildOfficeWorkerAssignmentCandidateGate(readiness)`, a blocked/display-only candidate gate for routed facilities (`orchestrator_desk`, `agent_desks`, `incident_corner`) that joins readiness, approval, audit, human confirmation, and authority blockers. The slice keeps `enabledControls: 0`, `assignmentCandidateEnabled: false`, `workAssignmentEnabled: false`, `requestCreationEnabled: false`, `dispatchEnabled: false`, and `auditWriteEnabled: false`; it creates no requests, promotes no candidates, assigns no work, enqueues nothing, dispatches nothing, writes no audit events, adds no mutation routes, services, forms, buttons, backend writes, Kanban transitions, projection promote/reject operations, watcher/cron enablement, service restarts, public exposure, renderer dependencies, or raw projection. The next product-improving slice should be `Worker Request Draft Preview 1`: show a hypothetical safe request draft shape derived from blocked candidates, still read-only and never persisted/created.
