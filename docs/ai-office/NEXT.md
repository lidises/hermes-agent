...
`Desk RPG Room Shell 1` is now locally implemented in `web/src/pages/OfficePage.tsx` with focused coverage in `web/src/pages/OfficePage.rpg.test.tsx`. `/office` renders the `buildOfficeDeskRpgProjectionModel(state)` DTO as a read-only DOM/CSS operating-room shell with stable `data-office-desk-rpg-*` hooks for the shell, facilities, actors, board/evidence/vault/ops posture, `enabledControls: 0`, `safeProjectionOnly: true`, `rawExcluded: true`, zero room-shell buttons, zero forms, and browser-smoked raw-leak false. The next product-improving slice should be `Desk RPG Inspector Migration 1`: move/bridge selected safe details into the right-inspector posture using the room-shell DTO without adding backend/schema/mutation/renderer-dependency expansion.

Master Spec v0.1 follow-up: read `docs/ai-office/plans/2026-05-14-desk-rpg-master-spec-review.md` before choosing the next work. It reframes read-only-first as the first safety posture, not the permanent product ceiling. The completed product-contract sequence is `Desk RPG Product Vision 1` → `Desk RPG Projection Model 1` → `Desk RPG IA/Layout 1` → `Controlled Mutation & Approval Model 1`; `Implementation Roadmap 1` reconciled those contracts into the first safe implementation sequence; `Desk RPG Projection ViewModel Helper 1` provides the pure helper foundation; `Desk RPG Room Shell 1` now renders it as the read-only operating-room shell, and the next step is `Desk RPG Inspector Migration 1` before any executable control.
