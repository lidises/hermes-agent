...
`Desk RPG Product Vision 1` is now documented in `docs/ai-office/product/desk-rpg-product-vision.md` after the controlled-mutation execution readiness summary. It defines the quiet JRPG operating-room target, read-only-first as the first safety posture rather than a permanent ceiling, the MVP success scene, six starting actors (User Avatar/Boss, Orchestrator, Search Worker, Reviewer, Wiki Writer, NAS Keeper), the single-office facility layout, Orchestrator-mediated instruction/quick-action principles, safe speech/redaction rules, event/projection posture, NAS Keeper final-save boundary, renderer non-adoption posture, and non-goals. The next product-improving slice should be `Desk RPG Projection Model 1`: define role avatar vs runtime instance, runtime/intent/visual projection event separation, character/desk/board/vault state, visible worker caps, noise suppression, and raw-leak boundaries before more UI implementation.
