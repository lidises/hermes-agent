...
`Desk RPG IA/Layout 1` is now documented in `docs/ai-office/product/desk-rpg-ia-layout.md` after `Desk RPG Projection Model 1`. It defines the single fixed central office layout, global status ribbon, boss desk, Orchestrator desk, worker desk cluster, central Kanban/Paperclip board, right inspector, NAS vault entrance, security/ops corner, calm activity lane, existing HUD/strip disposition categories, read-only navigation model, accessibility/reduced-motion posture, renderer non-adoption posture, and safe visual vocabulary. The next product-improving slice should be `Controlled Mutation & Approval Model 1`: define how natural-language instructions and worker quick actions become event requests, how Orchestrator mediates approval/hold/deny routing, and how NAS Keeper final-save authority/audit/rollback/dry-run/CLI confirmation posture works before any executable implementation.
