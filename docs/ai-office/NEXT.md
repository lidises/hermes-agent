...
`Worker Dispatch Dry-Run Envelope 1` is now implemented locally after `Worker Authority Handoff Envelope 1`. It adds `buildOfficeWorkerDispatchDryRunEnvelope(authorityHandoff)`, a display-only dry-run envelope for routed authority handoffs (`orchestrator_desk`, `agent_desks`, `incident_corner`) that lists the safe simulation scope, expected effects, rollback plan, and audit preview references required before any real dispatch can exist. The slice keeps `enabledControls: 0`, `dryRunExecutionEnabled: false`, `dispatchEnabled: false`, `adapterInstallationEnabled: false`, `requestCreationEnabled: false`, `workAssignmentEnabled: false`, and `auditWriteEnabled: false`; it runs no dry runs, executes nothing, dispatches nothing, installs no adapters, creates no requests, assigns no work, enqueues nothing, writes no audit events, adds no mutation routes, services, forms, buttons, backend writes, Kanban transitions, projection promote/reject operations, watcher/cron enablement, service restarts, public exposure, renderer dependencies, or raw projection. The next product-improving slice should be `Worker Audit Preview Envelope 1`: show the safe audit preview metadata that would be required after a dry-run envelope, still read-only and without writing audit events.
