...
Current local slice: `Event-driven Character State Projection 1` is implemented frontend-only/read-only. It adds `buildOfficeEventDrivenCharacterStateProjection(review, events)` and `EventDrivenCharacterStateProjectionPanel`, deriving six display-only Desk RPG character states (`user_boss`, `orchestrator`, `search_worker`, `reviewer`, `wiki_writer`, `nas_keeper`) from allowlisted safe runtime/intent/visual event categories plus the existing `Desk RPG Read-only Chain Completion Review 1` DTO. Stable hooks include `data-office-event-driven-character-state-projection*` and `data-office-event-driven-character-state-role="user_boss|orchestrator|search_worker|reviewer|wiki_writer|nas_keeper"`. It keeps enabledControls=0, runtimeEventWriteEnabled=false, intentEventCreationEnabled=false, visualEventCreationEnabled=false, eventPersistenceEnabled=false, stateMachinePersistenceEnabled=false, requestCreationEnabled=false, workAssignmentEnabled=false, dispatchEnabled=false, auditWriteEnabled=false, nasSaveEnabled=false, safeProjectionOnly=true, and rawExcluded=true. No backend/schema/API route/service/Kanban/cron/VPS/NAS change was made. Next recommended frontend-only/read-only slice: `Character State Room Overlay 1` — project the six safe character states into the existing Desk RPG room/facility surface as non-interactive presence/stance markers, without adding event persistence, backend streams, animation state persistence, request creation, worker assignment, dispatch, audit writes, NAS saves, or mutation controls.
