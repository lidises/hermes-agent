...
`Controlled Mutation & Approval Model 1` is now documented in `docs/ai-office/architecture/controlled-mutation-approval-model.md` after `Desk RPG IA/Layout 1`. It defines request classes for natural-language instructions, worker quick actions, and system attention; Orchestrator mediation states/outputs; human decision types; authority candidates and risk classes; dry-run, audit sink, rollback, NAS Keeper, and CLI confirmation posture; browser allowed/forbidden interactions; redaction requirements; and a future DTO boundary sketch without implementing code. The next product-improving slice should be `Implementation Roadmap 1`: convert the four Master Spec contracts into a small implementation roadmap and choose the first safe implementation slice without adding executable controls.

Master Spec v0.1 follow-up: read `docs/ai-office/plans/2026-05-14-desk-rpg-master-spec-review.md` before choosing the next work. It reframes read-only-first as the first safety posture, not the permanent product ceiling. The completed product-contract sequence is `Desk RPG Product Vision 1` → `Desk RPG Projection Model 1` → `Desk RPG IA/Layout 1` → `Controlled Mutation & Approval Model 1`; the next step is `Implementation Roadmap 1`, and any first implementation slice should be reconciled with those contracts before adding executable control.
