...
`Worker Audit Preview Envelope 1` is now implemented locally after `Worker Dispatch Dry-Run Envelope 1`. It adds `buildOfficeWorkerAuditPreviewEnvelope(dryRunEnvelope)`, a display-only audit preview envelope for routed dry-run envelopes (`orchestrator_desk`, `agent_desks`, `incident_corner`) that lists the safe dry-run reference, audit sink, event type, redaction policy, and rollback reference required before any audit event can be written. The slice keeps `enabledControls: 0`, `auditWriteEnabled: false`, `executionEnabled: false`, `dispatchEnabled: false`, `adapterInstallationEnabled: false`, `requestCreationEnabled: false`, and `workAssignmentEnabled: false`; it writes no audit events, executes nothing, dispatches nothing, installs no adapters, creates no requests, assigns no work, enqueues nothing, adds no mutation routes, services, forms, buttons, backend writes, Kanban transitions, projection promote/reject operations, watcher/cron enablement, service restarts, public exposure, renderer dependencies, or raw projection. The next product-improving slice should be `Worker Rollback Preview Envelope 1`: show the safe rollback preview metadata that would be required after audit preview, still read-only and without executing rollback or dispatch.
