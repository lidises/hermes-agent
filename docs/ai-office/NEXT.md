...
Last updated: 2026-05-15 17:57 KST
...
Current local slice: `Character Facility Role Legend 1` is implemented frontend-only/read-only. It adds `buildOfficeCharacterFacilityRoleLegend(summary, overlay)` and `CharacterFacilityRoleLegendPanel`, composing only the safe `Character Panel Boundary Summary 1` and `Character State Room Overlay 1` DTOs into a compact six-role legend that maps `user_boss`, `orchestrator`, `search_worker`, `reviewer`, `wiki_writer`, and `nas_keeper` to display-only facility zones (`boss_desk`, `orchestrator_desk`, `worker_cluster`, `right_inspector`, `central_board`, `nas_vault`). Stable hooks include `data-office-character-facility-role-legend*`, per-role hooks, and per-facility hooks. Boundary labels are safe static labels: `instruction display only`, `mediation display only`, `research posture only`, `review posture only`, `wiki draft disabled`, and `NAS save disabled`. It keeps enabledControls=0, clickHandlerEnabled=false, keyboardHandlerEnabled=false, formControlEnabled=false, eventPersistenceEnabled=false, backendStreamEnabled=false, animationStatePersistenceEnabled=false, requestCreationEnabled=false, workAssignmentEnabled=false, dispatchEnabled=false, auditWriteEnabled=false, nasSaveEnabled=false, safeProjectionOnly=true, and rawExcluded=true. No backend/schema/API route/service/Kanban/cron/VPS/NAS change was made. Next recommended frontend-only/read-only slice after verification/commit: `Character Facility Boundary Strip 1` — add a compact safe strip summarizing facility-zone posture and disabled mutation boundaries from the existing legend/overlay only, without exposing raw prompt/task body/transcript/path/source title/provider and without adding click handlers, keyboard handlers, forms/buttons/inputs, event persistence, backend streams, animation-state persistence, request creation, assignment, dispatch, audit writes, or NAS saves.
