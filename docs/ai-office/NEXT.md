...
`Worker Request Draft Preview 1` is now implemented locally after `Worker Assignment Candidate Gate 1`. It adds `buildOfficeWorkerRequestDraftPreview(candidateGate)`, a hypothetical request-draft preview for routed facilities (`orchestrator_desk`, `agent_desks`, `incident_corner`) derived only from blocked safe candidates. The slice keeps `enabledControls: 0`, `requestCreationEnabled: false`, `requestPersistenceEnabled: false`, `workAssignmentEnabled: false`, `dispatchEnabled: false`, and `auditWriteEnabled: false`; it creates no requests, persists no drafts, promotes no candidates, assigns no work, enqueues nothing, dispatches nothing, writes no audit events, adds no mutation routes, services, forms, buttons, backend writes, Kanban transitions, projection promote/reject operations, watcher/cron enablement, service restarts, public exposure, renderer dependencies, or raw projection. The next product-improving slice should be `Worker Human Confirmation Envelope 1`: show the safe confirmation envelope that would be required before any draft could become actionable, still read-only and without recording decisions.
