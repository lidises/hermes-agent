...
`Desk RPG Inspector Migration 1` is now locally implemented in `web/src/pages/OfficePage.tsx` with focused coverage in `web/src/pages/OfficePage.rpg.test.tsx`. `/office` now bridges `buildOfficeDeskRpgProjectionModel(state)` into a right-inspector posture via stable `data-office-desk-rpg-inspector-*` hooks for aggregate-only targets, board/evidence/vault/ops posture, `enabledControls: 0`, `safeProjectionOnly: true`, `rawExcluded: true`, zero inspector buttons, zero forms, and browser-smoked raw-leak false. The next product-improving slice should be `Desk RPG Board Evidence Tab 1`: migrate safe board/evidence details into a central board tab/posture without adding backend/schema/mutation/renderer-dependency expansion.

Master Spec v0.1 follow-up: read `docs/ai-office/plans/2026-05-14-desk-rpg-master-spec-review.md` before choosing the next work. It reframes read-only-first as the first safety posture, not the permanent product ceiling. The completed product-contract sequence is `Desk RPG Product Vision 1` → `Desk RPG Projection Model 1` → `Desk RPG IA/Layout 1` → `Controlled Mutation & Approval Model 1`; `Implementation Roadmap 1` reconciled those contracts into the first safe implementation sequence; `Desk RPG Projection ViewModel Helper 1` provides the pure helper foundation; `Desk RPG Room Shell 1` renders it as the read-only operating-room shell; `Desk RPG Inspector Migration 1` now bridges aggregate safe details into the right inspector, and the next step is `Desk RPG Board Evidence Tab 1` before any executable control.
