...
`Desk RPG Projection ViewModel Helper 1` is now locally implemented as a pure DTO helper in `web/src/pages/officeView.ts` with focused tests in `web/src/pages/OfficePage.test.ts`. It adds `buildOfficeDeskRpgProjectionModel(state)` and safe Desk RPG actors/facilities, board/evidence/vault/ops posture, inspector targets, suppression counts, `enabledControls: 0`, `safeProjectionOnly: true`, `rawExcluded: true`, and sentinel coverage for raw prompt/private path/token/traceback/provider exclusion. The next product-improving slice should be `Desk RPG Room Shell 1`: render this DTO as a read-only DOM/CSS operating-room shell without backend/schema/mutation/renderer-dependency expansion.

Master Spec v0.1 follow-up: read `docs/ai-office/plans/2026-05-14-desk-rpg-master-spec-review.md` before choosing the next work. It reframes read-only-first as the first safety posture, not the permanent product ceiling. The completed product-contract sequence is `Desk RPG Product Vision 1` → `Desk RPG Projection Model 1` → `Desk RPG IA/Layout 1` → `Controlled Mutation & Approval Model 1`; `Implementation Roadmap 1` reconciled those contracts into the first safe implementation sequence; `Desk RPG Projection ViewModel Helper 1` now provides the pure helper foundation, and the next step is the read-only `Desk RPG Room Shell 1` before any executable control.
